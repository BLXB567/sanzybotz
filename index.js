...
const starts = async (kurr = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('Miyuki DVR', {
		font: 'Standard',
...
console.log(color('> HARI ','silver'), color(`${tampilHari}\n`,'mediumseagreen'))
	kurr.browserDescription = [ 'Miyuki DVR', 'ubuntu', '3.0' ]
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `Miyuki DVR`, img,but = [{buttonId:`donasi`, 
			...
			try {
				...
				sendButImage(anu.jid, teks, `SanzyBotz-Assistan`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `LATOM !!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
...
kurr.on('group-update', async (anu) => {
    ...
    kurr.on('chat-update', async (message) => {
        ...
        ownerNumber = ["6283117436733@s.whatsapp.net","6281331493321@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
